...
//  MetroParis
//
//  Created by Bunti Nizama on 29/09/20.
                    ...
                    VStack {
                        if isDisplayMap == false {
                            List(records.indices,id: \.self) { index in
                                HStack{
                                    NavigationLink(destination: DetailView(record: self.records[index]).environment(\.managedObjectContext, self.managedObjectContext) ) {
