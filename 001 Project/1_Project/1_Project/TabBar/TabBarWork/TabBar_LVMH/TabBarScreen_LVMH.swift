...
struct TabBarScreen_LVMH: View {
    ...
    var body: some View {
        VStack(alignment: .center, spacing: 0) {
            ...
            ZStack {
                if self.viewRouterNeuromorphic.currentViewNeuromorphic == "home" {
                LoginScreen().environment(\.managedObjectContext, self.managedObjectContext)
                CatalogList().hidden()
                NewTabScreen().hidden()
                MagazineScreenView().hidden()

              } else if self.viewRouterNeuromorphic.currentViewNeuromorphic == "book"  {
                LoginScreen().hidden()
                CatalogList()
                NewTabScreen().hidden()
                MagazineScreenView().hidden()
              ...
              } else if self.viewRouterNeuromorphic.currentViewNeuromorphic == "News" {
               ...
               NewTabScreen()
                ...
                } else if self.viewRouterNeuromorphic.currentViewNeuromorphic == "list" {
                LoginScreen().hidden()
                CatalogList().hidden()
                NewTabScreen().hidden()
                 MagazineScreenView()
...
struct TabBarScreen_LVMH_Previews: PreviewProvider {
